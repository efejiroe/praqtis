...
# Attaches the practice's own PCN's QOF figure, the gap against it, and
# — critically — whether the practice is simply better-staffed than the
# rest of its network. A performance gap with a matching staffing gap is
# a resourcing story ("they have more people"); a performance gap with
# near-identical staffing intensity is a process story (the same
# resources doing something differently) — the second is the
# zero-extra-capacity finding worth leading a report with, and the first
# isn't, so this has to be checked before either framing is used, not
# assumed.
practice_vs_pcn <- function(snapshot, pcn_qof, pcn_workforce, pcn_list_size) {
