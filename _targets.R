...
list(
  ...
  tar_target(pcn_iif_indicators, fetch_pcn_iif_indicators()),

  # Aggregate to PCN, for the pilot ICB (no modelling yet — Phase 2)
  tar_target(
    pilot_pcn_list_size,
    pcn_list_size_for_icb(pcn_registration, epcn_mapping, pilot_icb_code)
  ),
  tar_target(
    pilot_pcn_imd,
  ...
  tar_target(
    pilot_pcn_iif,
    pcn_iif_for_icb(pcn_iif_indicators, epcn_mapping, pilot_icb_code)
  )
)
